//! Test suite for Lua VM closures and new opcodes
//! 
//! This test suite validates that our non-recursive, transaction-based Lua VM
//! properly implements closures, upvalues, and the new opcodes.

use ferrous::lua::{LuaVM, Value, value::{FunctionProto, Closure, CallFrame, UpvalueInfo, Table}, 
                   handle::{TableHandle, StringHandle, ClosureHandle}, transaction::HeapTransaction};
...
fn test_close_opcode() {
    ...
    let main_bytecode = vec![
        ...
        encode_abc(28, 2, 1, 1),    // 5: CALL R(2) := R(1)() ; FIXED: Use R(1) for closure
...
fn test_extraarg_opcode() {
    ...
    let main_bytecode = vec![
        ...
        encode_extra_arg(1),            // 4: EXTRAARG 1 (C value for SETLIST) FIXED: Use proper encoding
        encode_abc(30, 0, 2, 0),        // 5: RETURN R(0)
    ];
